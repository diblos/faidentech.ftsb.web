            ...
            <form action="insert.php" method="post">
                ...
                <div class="user-box">
                  ...
                  <div class="form-group">
                    <label><h5 style="color:white">CATEGORY</h5></label>
                    <select name="category" id="input-4" class="form-control" style="background-color: rgba(0,0,0,.3);">
    ...
    <section class="login-box container-fluid py-1">
      <div class="row">
        ...
        <div class="col-3 text-right pt-3 pb-0" style="text-align: right;">
          ...
          <input class="btn btn-light ml-1" type="button" value="New Record" onclick="toggleObjectVisibility(`newForm`, true);" title="New Record">
        </div>
      </div>
       <!-- FORM FILTER -->
       <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
        <div id="filter" class="row d-none">
          <div class="col">
            <div class="user-box">
                <input type="text" name="fname" id="input-1">
                <label>ENTER PERSON NAME</label>
            </div>
          </div>
          <div class="col">
            <div class="user-box">
                <input type="text" name="flicense" id="input-1">
                <label>LICENSE PLATE</label>
            </div>
          </div>
          <div class="col">
              <div class="form-group">
                    <label><h5 style="color:white">CATEGORY</h5></label>
                    <select name="fcategory" id="input-4" class="form-control" style="background-color: rgba(0,0,0,.3);">
    ...
    <table cellpadding="0" cellspacing="0" border="0">
      <thead>
        <tr>
          ...
          <th class="w20 p-2">License Plate</th>
    ...
    if($fname=='' && $flicense=='' && $fcategory==''){
      ...
      if($getData->num_rows > 0) {
       ...
       while($row = $getData->fetch_assoc()) {
        ...
        $found = find($options, 'value', $category);
        $categoryV = $found==null? $category : $found->label;

        $id = $row["id"];

        $fstr = joinStrings('|', $name, $license, $category);
        ...
        $optionstr = '';
        foreach($options as $option) {
          $selected = ($option->value==$category)?' selected':'';
          $optionstr = $optionstr.'<option value="'.$option->value.'" '.$selected.'>'.$option->label.'</option>';
        }

        echo '
              <form name="sel'.$id.'-form" action="update_1.php" method="post">
              ...
              <tr class="rownum">
                  ...
                  <td class="p-1" scope="row">
                    <span id="sel'.$id.'-nameV" class="d-block">'.$name.'</span>
                    <span id="sel'.$id.'-nameI" class="d-none">
                      <input type="text" name="name" value="'.$name.'" placeholder="Name">
                    </span>
                  </td> 
                  <td class="w20 p-1">
                    <span id="sel'.$id.'-licenseV" class="d-block">'.$license.'</span>
                    <span id="sel'.$id.'-licenseI" class="d-none">
                      <input type="text" name="license" value="'.$license.'" placeholder="License">
                    </span>
                  </td> 
                  <td class="w15 p-1">
                    <span id="sel'.$id.'-categoryV" class="d-block">'.$categoryV.'</span>
                    <span id="sel'.$id.'-categoryI" class="d-none">
                      <select name="category" id="input-4" class="form-control" style="background-color: rgba(0,0,0,.3);">
                        '.$optionstr.'
                      </select>
                    </span>
                  ...
                  <td class="w20 p-1">
                      ...
                      <a id="sel'.$id.'-delete" class="btn btn-dark" role="button" href="delete_1.php?id='.$id.'"><i class="fa fa-trash"></i></a>
        ...
        // <a id="sel'.$id.'-save" class="btn btn-dark disabled" role="button" href="#" onclick="editMode(\'sel'.$id.'\', false);return false;"><i class="fa fa-save"></i></a>
        // <button id="sel'.$id.'-save" class="btn btn-dark disabled" type="submit"><i class="fa fa-save"></i></button>
        $rownum = $rownum + 1;
       }
      } else {
        echo '<tr> 
        <td colspan="5" class="text-center p-1"> No Records </td>
        </tr>';
      }
?>
      </tbody>
  ...
  <div class="row">
      <div class="col pt-4 pb-0">
        ...
        <form name="download" method="post" action="../assets/php/export_authorize.php">
<?php
